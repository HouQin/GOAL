...
% % insert image
% train_data = func_insertImage(train_data, 40, 40, 'monkey.png');
...
% dig a hole
train_data = reshape(train_data,[40 40 train_n]);
train_data = func_digahole(train_data,holesize);
%             if necessary, figure can be applied here.
train_data = reshape(train_data,[1600,train_n]);
...
for i=1:length(dim)
    for j = 1: length(sigma_mul)
        ...
        for t = 1:400
            % 生成两个不重复的随机整数
            random_numbers = randbenford(2, 100, 1, 0, 1);
            new_baby = zeros(1, 5);
